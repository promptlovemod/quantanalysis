...
def build_quality_gate(stock_data: dict, thresholds: dict | None = None) -> dict:
    ...
    wf_count = 0
    cpcv_count = 0
    wf_scope_eligible_count = 0
    cpcv_scope_eligible_count = 0
    seed_count = 0
    reliability_count = 0
    calibration_count = 0
    buy_recall_count = 0
    ...
    for ticker in tickers:
        ...
        wf_scope_eligible = has_wf and (legacy_evidence_scope or wf_scope_selected)
        cpcv_scope_eligible = has_cpcv and (legacy_evidence_scope or cpcv_scope_selected)

        wf_count += int(has_wf)
        cpcv_count += int(has_cpcv)
        wf_scope_eligible_count += int(wf_scope_eligible)
        cpcv_scope_eligible_count += int(cpcv_scope_eligible)
        ...
        wf_sharpes.append(wf if wf_scope_eligible else None)
        cpcv_p5.append(cpcv if cpcv_scope_eligible else None)
        ...
        if signal_present and legacy_evidence_scope:
            warnings.append("legacy_evidence_scope_assumed")
        ...
        per_ticker[ticker] = {
            ...
            "evidence_sources": {
                ...
                "wf_scope_eligible": wf_scope_eligible,
                "cpcv_scope_eligible": cpcv_scope_eligible,
            },
            "warnings": warnings,
    ...
    metric_values_by_check = {
        "min_success_rate": metrics["success_rate"],
        "min_positive_wf_share": metrics["positive_wf_share"],
        "min_median_wf_sharpe": metrics["median_wf_sharpe"],
        "min_median_cpcv_p5": metrics["median_cpcv_p5"],
        "min_seed_stable_rate": metrics["seed_stable_rate"],
        "min_reliability_score_mean": metrics["reliability_score_mean"],
        "max_median_ece": metrics["median_ece"],
        "min_median_buy_recall": metrics["median_buy_recall"],
    }
    insufficient_evidence_checks = [
        f"insufficient_evidence:{name}" for name, value in metric_values_by_check.items() if value is None
    ]
    ...
    coverage = {
        ...
        "walkforward_scope_eligible": _coverage_stat(wf_scope_eligible_count, len(tickers)),
        "cpcv_scope_eligible": _coverage_stat(cpcv_scope_eligible_count, len(tickers)),
    ...
    data_availability = {
        "wf_sharpe": {"count": int(wf_scope_eligible_count), "total": len(tickers), "available": wf_scope_eligible_count >= 1},
        "cpcv_p5": {"count": int(cpcv_scope_eligible_count), "total": len(tickers), "available": cpcv_scope_eligible_count >= 1},
        "reliability_score_mean": {"count": int(reliability_count), "total": len(tickers), "available": reliability_count >= 1},
        "median_ece": {"count": int(calibration_count), "total": len(tickers), "available": calibration_count >= 1},
        "median_buy_recall": {"count": int(buy_recall_count), "total": len(tickers), "available": buy_recall_count >= 1},
    }
