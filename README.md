...
- `TELEGRAM_ALERT_SUPPRESSION_SECONDS`
- `TELEGRAM_STATE_DIR`
...
- `TELEGRAM_RATE_LIMIT_SECONDS` adds a minimum pause between Telegram API calls
- `TELEGRAM_ALERT_SUPPRESSION_SECONDS` suppresses duplicate success/failure alerts inside the configured window
- notifier delivery state is stored under `reports/.telegram/` unless `TELEGRAM_STATE_DIR` is set
...
- `<TICKER>_orchestration_summary.json`
- `<TICKER>_master.log`
...
- `reports/portfolio_orchestration_summary.json`
