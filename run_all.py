...
from utils.orchestration_summary import (
    summarize_portfolio_run,
...
<tr class="{'row-fail' if r['failed'] else ''}">
  ...
  <div class="sum-label">suggested</div></div>"""
    ...
    dashboard_path = out_dir / "portfolio_dashboard.html"
    orchestration_summary = summarize_portfolio_run(
        tickers,
        failed,
        portfolio_summary,
    ...
    write_orchestration_summary(out_dir / "portfolio_orchestration_summary.json", orchestration_summary)
    orchestration_color = {
        'OK': '#00C853',
        'WARNING': '#FFD600',
        'FAILED': '#D50000',
    }.get(orchestration_summary.get('overall_status'), '#58A6FF')
    orchestration_notice = (
        f'<div class="card" style="border-color:{orchestration_color}40">'
        f'<h2>Run Orchestration</h2>'
        f'<div class="metric-row"><span class="metric-label">Overall status</span>'
        f'<span class="metric-val" style="color:{orchestration_color}">{orchestration_summary.get("overall_status","N/A")}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Failed tickers</span>'
        f'<span class="metric-val">{len(orchestration_summary.get("failed_tickers", []))}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Pipeline-failed tickers</span>'
        f'<span class="metric-val">{len(orchestration_summary.get("pipeline_failed_tickers", []))}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Quality gate</span>'
        f'<span class="metric-val">{orchestration_summary.get("quality_gate_status","N/A")}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Allocation status</span>'
        f'<span class="metric-val">{orchestration_summary.get("allocation_status","N/A")}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Actionable universe</span>'
        f'<span class="metric-val">{orchestration_summary.get("actionable_universe_size",0)}/{len(tickers)}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Missing artifacts</span>'
        f'<span class="metric-val">{orchestration_summary.get("missing_artifact_count",0)}</span></div>'
        f'<div class="note">Artifact summary: reports/portfolio_orchestration_summary.json</div>'
        f'</div>'
    )
    visual_sections = ''
    if optimizer_img:
...
def build_single_dashboard(ticker: str, module_errors: dict) -> Path:
    ...
    dashboard_path = out_dir / f"{ticker}_dashboard.html"
    orchestration_summary = summarize_single_run(
        ticker,
        out_dir,
        module_errors,
        signal_json=signal,
    ...
    write_orchestration_summary(out_dir / f"{ticker}_orchestration_summary.json", orchestration_summary)
    ...
    orchestration_module_text = ', '.join(
        f'{name}={"OK" if ok else "FAIL"}'
        for name, ok in (orchestration_summary.get('module_status', {}) or {}).items()
    ) or 'N/A'
    orchestration_section = (
        f'<div class="card"><h2>Pipeline Orchestration</h2>'
        f'<div class="metric-row"><span class="metric-label">Overall status</span>'
        f'<span class="metric-val" style="color:{orchestration_color}">{orchestration_summary.get("overall_status","N/A")}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Modules</span>'
        f'<span class="metric-val">{orchestration_summary.get("module_success_count",0)}/{orchestration_summary.get("module_total",0)}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Module detail</span>'
        f'<span class="metric-val">{orchestration_module_text}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Pipeline status</span>'
        f'<span class="metric-val">{orchestration_summary.get("pipeline_status","N/A")}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Pipeline errors</span>'
        f'<span class="metric-val">{orchestration_summary.get("pipeline_error_count",0)}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Fatal pipeline errors</span>'
        f'<span class="metric-val">{orchestration_summary.get("fatal_pipeline_error_count",0)}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Consistency status</span>'
        f'<span class="metric-val">{orchestration_summary.get("dashboard_consistency_status","N/A")}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Invariant status</span>'
        f'<span class="metric-val">{orchestration_summary.get("artifact_invariant_status","N/A")}</span></div>'
        f'<div class="metric-row"><span class="metric-label">Missing artifacts</span>'
        f'<span class="metric-val">{orchestration_summary.get("missing_artifact_count",0)}</span></div></div>'
    )
...
{extra_chart_cards}
<div class="grid g3">{execution_section}{conf_section}{wf_section}{cpcv_section}{mc_rel_section}{spec_growth_section}{backtest_audit_section}{orchestration_section}</div>
{explain_section}
{mc_table_section}
<div class="grid g3">
