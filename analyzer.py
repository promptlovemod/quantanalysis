...
class DataFetcher:
    def __init__(self, ticker, period):
        ...
        rate_limiters = {
            # Tiingo free tier is tight enough that a small fixed interval is safer
            # than bursty retries when refreshing a batch universe.
    ...
    def _fetch_ohlcv(self, symbol: str, reuse_primary: bool = False) -> tuple:
        ...
        if symbol.startswith("^"):
            frame = self._download_yfinance_ohlcv(symbol)
            if frame is None or frame.empty:
                raise RuntimeError(f"Could not fetch data for {symbol}")
            DataCache.save(symbol, frame)
            return frame, {"cache_state": "miss", "source": "yfinance", "provider_attempts": []}

        return self._market_data_service.fetch_daily_ohlcv(symbol, self.period)
...
class RegimeModel:
    ...
    @staticmethod
    def _component_log_likelihood(obs: np.ndarray,
                                  means: np.ndarray,
        ...
        for mean, cov in zip(means, covars):
            ...
            if sign <= 0:
                cov = cov + np.eye(n_features) * 1e-5
                sign, logdet = np.linalg.slogdet(cov)
            inv = np.linalg.pinv(cov)
            diff = obs - mean.reshape(1, -1)
            maha = np.einsum('ij,jk,ik->i', diff, inv, diff)
            out.append(-0.5 * (maha + logdet + n_features * np.log(2.0 * np.pi)))
        return np.column_stack(out)

    def _classify_hmm_observations(self, obs_df: pd.DataFrame) -> pd.Series:
        if self.hmm_ is None or obs_df is None or obs_df.empty:
            return pd.Series(dtype=int, name='regime')
        cols = self.obs_cols_ if self.obs_cols_ else list(obs_df.columns)
        obs = obs_df[cols].values
        log_likelihood = self._component_log_likelihood(
            obs,
            getattr(self.hmm_, 'means_', np.zeros((1, obs.shape[1]))),
            getattr(self.hmm_, 'covars_', np.tile(np.eye(obs.shape[1]), (1, 1, 1))),
        )
        states = log_likelihood.argmax(axis=1)
        return pd.Series(
    ...
    # ── HMM fitting ────────────────────────────────────────────────
    def _fit_hmm(self, fit_end=None) -> 'pd.Series | None':
        if not HAS_HMM:
        ...
        if fit_end is not None:
            ...
            if getattr(obs_df.index, 'tz', None) is not None and cutoff.tzinfo is None:
                cutoff = cutoff.tz_localize(obs_df.index.tz)
            elif getattr(obs_df.index, 'tz', None) is None and cutoff.tzinfo is not None:
                cutoff = cutoff.tz_convert(None)
            fit_obs_df = obs_df.loc[obs_df.index <= cutoff]
        min_rows = int(self.cfg.get('regime_min_rows', 252))
        if len(fit_obs_df) < min_rows:
            log.warning(
                f"  HMM fit skipped: historical fit rows={len(fit_obs_df)} "
                f"(<{min_rows}) before train/test cutoff"
            )
            return None
        obs = fit_obs_df.values
        try:
            ...
            means  = [
                float(obs[states == s, 0].mean()) if np.any(states == s)
                else float(np.asarray(model.means_)[s, 0])
                for s in range(nc)
            ]
            ...
            regimes = self._classify_hmm_observations(obs_df)
            bull_pct = (regimes == 1).mean()
            cutoff_msg = f"  fit_rows={len(fit_obs_df)}" if fit_end is not None else ""
            log.info(f"  HMM: mode={mode}{cutoff_msg}  bull={bull_pct*100:.0f}%  "
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> bool:
        ...
        if sp <= 0:
            return False
        train_cutoff = X.index[min(sp - 1, len(X.index) - 1)]
        regimes = self._fit_hmm(fit_end=train_cutoff)
